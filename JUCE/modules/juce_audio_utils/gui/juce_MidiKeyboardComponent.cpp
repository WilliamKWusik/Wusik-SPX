...
int MidiKeyboardComponent::xyToNote (Point<float> pos, float& mousePositionVelocity)
{
	...
	if (pos.x < 0 || pos.y < 0 || pos.x > getWidth() || pos.y > getHeight()) return -1;

	//AlertWindow::showMessageBox(AlertWindow::NoIcon, String(pos.x) + "/" + String(pos.y), getBounds().toString());
...
void MidiKeyboardComponent::updateNoteUnderMouse (const MouseEvent& e, bool isDown)
{
	...
	//AlertWindow::showMessageBox(AlertWindow::NoIcon, "", String(e.position.toString()));

    //updateNoteUnderMouse (e.getEventRelativeTo (this).position, isDown, e.source.getIndex());
	updateNoteUnderMouse(e.position, isDown, e.source.getIndex());
}
...
void MidiKeyboardComponent::updateNoteUnderMouse (Point<float> pos, bool isDown, int fingerNum)
{
    ...
    if (isDown)
    {
        if (newNote != oldNoteDown)
        {
            ...
            {
				//AlertWindow::showMessageBox(AlertWindow::NoIcon, String(mousePositionVelocity, 4), String(velocity, 4));

				//if (useMousePositionForVelocity) AlertWindow::showMessageBox(AlertWindow::NoIcon, String(newNote), String(mousePositionVelocity));
...
void MidiKeyboardComponent::mouseMove (const MouseEvent& e)
{
	...
	//AlertWindow::showMessageBox(AlertWindow::NoIcon, "", e.getPosition().toString());
...
void MidiKeyboardComponent::mouseDown (const MouseEvent& e)
{
	...
	//AlertWindow::showMessageBox(AlertWindow::NoIcon, String(e.position.x) + "/" + String(e.position.y) + " - " + String(newNote), String(mousePositionVelocity, 4));
	//AlertWindow::showMessageBox(AlertWindow::NoIcon, String(getWidth()), String(getHeight()));
...
bool MidiKeyboardComponent::setKey(const KeyPress& key)
{
	for (int i = keyPresses.size(); --i >= 0;)
	{
		if (keyPresses.getReference(i).getKeyCode() == key.getKeyCode())
		{
			keyPresses.getReference(i) = key;
