...
static bool canBeTriggered (const PopupMenu::Item& item) noexcept        { return item.isEnabled && !item.isSectionHeader && item.subMenu == nullptr; } // item.itemID != 0 && 
...
struct MenuWindow  : public Component
{
    ...
    void calculateWindowPos (Rectangle<int> target, const bool alignToRectangle)
    {
		...
		if (parent == nullptr)
		{
			Point<int> xMousePos = getMouseXYRelative();
			//
			if (x >= xMousePos.x) x -= 12; else x += 12;
			if (y >= xMousePos.y) y -= 12; else y += 12;
		}
...
private:
    ...
    void handleMousePosition (Point<int> globalMousePos)
    {
		...
		if (window.hasBeenOver && !isOverAny)  // window.hideOnExit && 
		{
			//window.unfocusAllComponents();
...
#if !WALTERNATIVEMENU
		if (wasDown && timeNow > window.windowCreationTime // + 250
                                         ...
                                         bool canBeModal)
{
	...
	if (items.size() > 0)
	{
		Item i;
		i.isSeparator = true;
		items.insert(items.begin(), std::move(i));

		Item i2(std::move("[X] Close"));
		i2.isEnabled = true;
		i2.isTicked = false;
		i2.isSeparator = false;
		i2.itemID = 0;
		items.insert(items.begin(), std::move(i2));
	}
	else return 0;
	#endif
