...
struct Component::ComponentHelpers
{
    ...
    //==============================================================================
    static inline bool hitTest (Component& comp, Point<int> localPoint, bool isFileDrop)
    {
        return isPositiveAndBelow (localPoint.x, comp.getWidth())
            && isPositiveAndBelow (localPoint.y, comp.getHeight())
            && comp.hitTest (localPoint.x, localPoint.y, isFileDrop);
    }
...
//==============================================================================
bool Component::hitTest (int x, int y, bool isFileDrop)
{
    ...
    if (flags.allowChildMouseClicksFlag)
    {
        for (int i = childComponentList.size(); --i >= 0;)
        {
            ...
            if (child.isVisible()
                 && ComponentHelpers::hitTest (child, ComponentHelpers::convertFromParentSpace (child, Point<int> (x, y)), isFileDrop))
                return true;
        }
    }
...
{
    if (ComponentHelpers::hitTest (*this, point, false))
    {
        if (parentComponent != nullptr)
...
Component* Component::getComponentAt (Point<int> position, bool isFileDrop)
{
    if (flags.visibleFlag && ComponentHelpers::hitTest (*this, position, isFileDrop))
    {
        for (int i = childComponentList.size(); --i >= 0;)
        {
            auto* child = childComponentList.getUnchecked(i);

            child = child->getComponentAt (ComponentHelpers::convertFromParentSpace (*child, position), isFileDrop);
